...
line = (
    ...
    .encode(
        ...
        y=alt.Y(
            ...
            scale=alt.Scale(
                domain=[
                    monthly_cpi_data["AVG_CPI"].min(),
                    monthly_cpi_data["AVG_CPI"].max(),
                ]
            ),
...
cpi_chart = (line + points).properties(width=800, height=400)
...
annual_wages_line = base.mark_line(color="#29B5E8").encode(
    y=alt.Y(
        ...
        scale=alt.Scale(
            domain=[
                annual_wages_cpi_data["AVG_ANNUAL_WAGES"].min() * 0.95,
                annual_wages_cpi_data["AVG_ANNUAL_WAGES"].max(),
            ]
        ),
...
cpi_line = base.mark_line(color="#D45B90").encode(
    y=alt.Y(
        ...
        scale=alt.Scale(
            domain=[
                annual_wages_cpi_data["CPI"].min() * 0.95,
                annual_wages_cpi_data["CPI"].max(),
            ]
        ),
...
combined_chart = (
    alt.layer(annual_wages_line + annual_wages_points, cpi_line + cpi_points)
